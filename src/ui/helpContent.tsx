...
export const DominanceChartHelp: FC = () => (
	<>
		...
		<P>
			<b>Nodes in analysis.</b> By default analysis involves all planets and luminaries (except those in Ophiuchus if applicable). This can be changed in the settings to be all selected nodes or all selected physical nodes.
		</P>
	</>
);
...
export const OrientationHelp: FC = () => (
	<>
		...
		<P>
			<b>Nodes in analysis.</b> By default analysis involves all planets and luminaries. This can be changed in the settings to be all selected nodes or all selected physical nodes.
		</P>
	</>
);
...
export const NodeSelectorHelp: FC = () => (
	<>
		...
		<P>
			<b>Effects.</b> The nodes selected here will be the nodes that appear on the main chart, the nodes for which aspects are calculated, and the nodes that can be selected in the planet info panel. By default, this selection will not affect the nodes taken into consideration for element/mode analysis, orientation analysis, or rulership structure, but this can be changed in the settings.
		</P>
		<P>
		...
		<UL>
			...
			<li>Arabic parts - the more frequent ones in modern western astrology, all traced back to either Dorotheus of Sidon or Paulus Alexandrinus (in particular - DS and PA provide different definitions for a part of "Eros", we go with the Dorothean definition).</li>
			<li>The North and South Lunar Nodes, as well as the Lilith and Selene (the lunar apogee and perigee). Note that these are the mean (via Meeus) positions by default - this can be changed in the settings.</li>
...
export const GeneralSettingsHelp: FC = () => (
	<>
		<P>
			This is the settings panel, exposing options relating to which astrological conventions are followed, and to how the App is rendered visually.
		</P>
		<P>
			<b>Houses.</b> Allows for the selection of a specific house system. Also allows for toggling of the "house presweep" rule, where houses are defined to begin 5 degrees before their cusp.
		</P>
		<P>
		...
		<P>
			<b>Lunar Nodes.</b> Allows for the selection of lunar node calculation mode. "True" uses astronomical calculation to figure out the true instantaneous position of all lunar nodes. "Mean" uses Meeus' polynomials to obtain the mean, non-oscillating positions of the lunar nodes.
		</P>
		<P>
...
export const AspectMenuHelp: FC = () => (
	<>
		...
		<P>
			<b>Aspects displayed.</b> Information will be displayed about all aspects between selected nodes that match the filters specified in the settings. Configurations will be displayed first, ordered by amount of nodes. Beyond this, aspects are always ordered by orb (increasing).
		</P>
		<P>
			<b>Hierarchy.</b> There is a notion of an aspect being contained in another (e.g. a grand trine contains three trines). This panel uses this idea, in particular displaying subaspects below their parent configuration. This can be changed in the settings (showing all aspects in a flat list, or not showing any subaspects).
		</P>
		<P>
			<b>Configurations.</b> Configurations are displayed in a mini-diagram, with all the subaspects below. These can be hovered to highlight them both in the mini-diagram and main chart.
		</P>
		<P>
			<b>Binary aspects.</b> Binary aspects are displayed as a list before the configurations, listing their aspect kind, nodes, and orb.
		</P>
		<P>
			<b>Interactability.</b> Hovering over any aspect will highlight this aspect in the main chart. In particular hovering over a parallel/contraparallel will also show the line connecting the two equinoxes, analogous to the celestial equator. Aspects may be discarded by clicking on the "X" button, but they will come back upon recomputation (changes in date/location or settings that change node positions).
		</P>
	</>
);
